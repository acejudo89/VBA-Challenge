...
Sub stocks()
    ...
    Dim totalvol, openprice, closeprice, pricedif, pricevar As Double
        totalvol = 0
   'Set two index, i index will help searching data and j index will help to buid the summary table
        ...
        'Then it will increase the i index to the next row
           ...
           If ws.Cells(i + 1, ticker).Value = ws.Cells(i, ticker).Value Then
            ...
            Else
               ...
               End If
               ws.Cells(j, difference_var) = ws.Cells(j, difference_result).Value / openprice
...
'Reset both index, i index will help searching data and j index will help to buid the geatest values table
   ...
   'set haders for the resume table
    ...
    increase_volume = WorksheetFunction.Match(WorksheetFunction.Min(ws.Range("K2:K" & RowCount)), ws.Range("K2:K" & RowCount), 0)
